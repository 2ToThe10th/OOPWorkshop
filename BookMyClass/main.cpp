...
// реализовать класс который ведет себя как целое число, которое перехватывает все операции, возможность ставить limit
// класс как целое число, не хранящее данные, которые спрашивает данные у апользователя
// переписать на string view